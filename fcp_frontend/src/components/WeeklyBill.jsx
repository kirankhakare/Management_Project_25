      ...
      {report && report.entries.length > 0 && (
        <Paper id="billPreview" sx={{ p: 3, mb: 3, overflowX: "auto" }}>
          <Typography variant="h6" gutterBottom>
            User: {report.userName} | Date Range: {new Date(fromDate).toLocaleDateString("en-GB")} - {new Date(toDate).toLocaleDateString("en-GB")}
          </Typography>

          <table style={{ width: "100%", borderCollapse: "collapse", textAlign: "center", tableLayout: "fixed", marginTop: "15px", fontSize: "14px" }}>
            <thead>
            ...
            <tbody>
              {report.entries.map((e, i) => (
                <tr key={i} style={{ borderBottom: "1px solid #ddd", backgroundColor: i % 2 === 0 ? "#fafafa" : "white" }}>
                  <td style={{ padding: "8px" }}>{new Date(e.date).toLocaleDateString("en-GB")}</td>
